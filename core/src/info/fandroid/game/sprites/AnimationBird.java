...
public class AnimationBird {
    ...
    public AnimationBird(TextureRegion region, Texture texture, float cycleTime, int current_frame1, int k, int rowIndex1, String state1) {
        rowIndex = rowIndex1;
        frameRow = 2;
        frameCol = 5;
    ...
    public void update(float dt){
        ...
        if (currentFrameTime > maxFrameTime) {
            ...
            if (current_frame == 4){
                if (state == "Menu") {
                    MenuState.birdAnimationIsChanged(index, rowIndex);
                }
                else if (state == "Begin") {
                    BeginState.birdAnimationIsChanged(index, rowIndex);
                }
                else if (state == "Settings") {
                    SettingsState.birdAnimationIsChanged(index, rowIndex);
                }
                else if (state == "Cust") {
                    CustomizationState.birdAnimationIsChanged(index, rowIndex);
                }
                current_frame = -1;
            }
            current_frame++;
